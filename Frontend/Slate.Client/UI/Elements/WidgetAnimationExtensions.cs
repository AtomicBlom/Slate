...
namespace Slate.Client.UI.Elements
{
    ...
    {
        public static async Task FadeOutAsync(this Widget element, TimeSpan? duration = null, Easings.Easing? easing = null, bool remove = false, bool disable = true, CancellationToken? cancellationToken = null)
        {
            if (disable)
            {
            ...
            await Task.Run(async () =>
            {
                ...
                if (remove)
                {
                    await TaskDispatcher.FireOnUIAndForgetAsync(() =>
                    {
